...
import { 
  ...
  HelpCircle, Image as ImageIcon, CheckCircle2, RefreshCw
} from 'lucide-react';
      ...
      try {
        ...
        if (pRes.success && pRes.product) {
          ...
          setFormData({
            ...
            stockQuantity: p.stock?.availableQuantity ?? p.stockQuantity ?? p.stock?.quantity ?? 10,
            SKU: p.SKU || '',
            modelNumber: p.modelNumber || '',
            gstRate: p.gstRate || '18',
            warrantySummary: p.warrantySummary || '1 Year Brand Manufacturer Warranty',
            ...
            offerActive: Boolean(p.offer?.isActive),
            offerDiscountPercent: p.offer?.discountPercent || '',
            offerLabel: p.offer?.label || '',
          });

          // Images
          const rawImgs = p.images || [];
          setImages(rawImgs.map((img) => (typeof img === 'string' ? img : img.url)));
          ...
          if (p.specifications && typeof p.specifications === 'object') {
            ...
            setSpecs(specRows.length > 0 ? specRows : [{ key: 'Processor', value: '' }]);
          } else {
            setSpecs([{ key: 'Processor', value: '' }]);
          }
        }
      } catch (err) {
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              Update pricing, live inventory, photo angles, and technical specifications.
            </p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs space-y-5">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div>
              ...
              <p className="text-xs text-slate-500">
                Upload real product images from your computer.
              </p>
            </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="space-y-1.5">
              ...
              {isCreatingBrand ? (
                ...
                >
                  ...
                  {brandsList.map((b) => (
                    <option key={b._id || b.slug} value={b._id}>
                      {b.name}
                    </option>
                  ))}
                </select>
            ...
            {/* SKU */}
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-700">SKU / Product ID</label>
              <input
                type="text"
                name="SKU"
                value={formData.SKU}
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs space-y-5">
          <div className="flex justify-between items-center border-b border-slate-100 pb-3">
            <div>
              ...
              <p className="text-xs text-slate-500">Key technical parameters.</p>
            </div>
            <button
              type="button"
          ...
          <div className="space-y-3">
            {specs.map((row, idx) => (
              <div key={idx} className="flex items-center space-x-3">
                ...
                <input
                  type="text"
                  placeholder="Value (e.g. RTX 4080 12GB)"
                  value={row.value}
